...
Repeating annotation are used when you want to apply the same annotation to a declaration or type use.
...
Two steps are required to create a repeating annotation:
1. Annotate your annotation with the `@Repeateable`. The `@Repeateable` expects you to send a container annotation for your annotation.
2. Create a container annotation, that has as `value` the array type of your annotation. The component type of the array type must be the repeatable annotation type.

### Retrieving Annotations
There are several methods available in the Reflection API that can be used to retrieve annotations. For example, `AnnotatedElement.getAnnotation(Class<T>)` that returns a single annotation if the annotation of the request type is present.

If more than one annotation is present, because of repeating annotations, you can obtain them by first getting container annotation. For example, the method `AnnotatedElement.getAnnotationsByType(Class<T>)` which returns an array of annotations.
